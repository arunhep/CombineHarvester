...
class MSSMHiggsModel(PhysicsModel):
    ...
    def buildModel(self):
        ...
        mA = ROOT.RooRealVar('mA', 'm_{A} [GeV]', 130.)
		...
		if X == 'H': self.doHistFunc('xs_qq%s_%s' % (X, era), f.Get(hd['xs_qq%s'%X]), pars)
            #! [part3]
                ...
                self.PROC_SETS.append(([ 'gg%s'%X, 'bb%s'%X ], [ '%stautau'%X, '%sbb'%X, '%sWW'%X], [era])
            )
	...
	print self.DC.bins
	    ...
	    print self.DC.exp[bin].keys()
            for proc in self.DC.exp[bin].keys():
                if self.DC.isSignal[proc] and 'SM125' not in proc:
