...
class MSSMLikeHiggsModel(PhysicsModel):
    ...
    def getYieldScale(self,bin,process):
        ...
        if processSource not in ["ggh", "bbh", "ggA", "bbA", "ggH", "qqH"]:
            raise RuntimeError, "Validation Error: signal process %s not among the allowed ones." % processSource
        foundDecay = None
        for D in [ "htautau", "Atautau", "Htautau", "hbb", "htt", "hmm", "HWW", "INTHWW"]:
            if D in decaySource:
                if foundDecay and decaySource!="INTHWW": raise RuntimeError, "Validation Error: decay string %s contains multiple known decay names" % decaySource
                foundDecay = D
        ...
        for D in [ "7TeV", "8TeV", "13TeV" ]:
            if D in decaySource:
                ...
                foundEnergy = D
        if not foundEnergy:
            foundEnergy = "13TeV" ## To ensure backward compatibility
...
class FloatingMSSMXSHiggs(MSSMLikeHiggsModel):
    """
    Trivial model to float ggH and qqH independently. At the moment only ggH and bbh are supported. Extensions to the other
    ...
    def doParametersOfInterest(self):
        ...
        """
        ## Example: evaluate cross sections for given values of tanb and mA, the names of the variables are ggH_xsec and qqH_xsec,
        ...
        #self.modelBuilder.doVar("qqH_xsec[%g]" % (mssm_xsec['higgses']['A']['xsec']['santander']*mssm_xsec['higgses']['A']['BR']))
        #self.modelBuilder.doVar("ggH_xsec[%g]" % (mssm_xsec['higgses']['A']['xsec']['ggF'      ]*mssm_xsec['higgses']['A']['BR']))
        #
        ## Define signal strengths on ggH and qqH as POI, NOTE: the range of the POIs is defined here
        self.modelBuilder.doVar("r_ggH[%s,%s,%s]" % (str((float(self.ggHRange[0])+float(self.ggHRange[1]))/2.), self.ggHRange[0], self.ggHRange[1]));
        #self.modelBuilder.doVar("r_qqH[%s,%s,%s]" % (str((float(self.qqHRange[0])+float(self.qqHRange[1]))/2.), self.qqHRange[0], self.qqHRange[1]));
    ...
    def getHiggsSignalYieldScale(self,production,decay, energy):
        if production == "ggH" or production == "qqH":
            ## This is an example how to multiply the yields r_ggH and r_qqH with the roofit variables ggH_xsec and qqH_xsec
            ...
            ## trivial renaming, but we leave it in as an example. Instead also r_ggH and r_qqH could be passed on directly
            ## in the return function instead of the newly defined variables ggH_yields or qqH_yield.
            if decay == "INTHWW":
                self.modelBuilder.factory_('expr::%sINT_yield("sqrt(@0)", r_%s)' % (production, production))
                return "%sINT_yield" % production
            else:
