...
class THDMHiggsModel(PhysicsModel):
	...
	if name == "xs_ggHINT_13TeV":
		...
		for j in range(hist.GetYaxis().GetNbins()):
		    #print i,j,hist.GetBinContent(i,j)
		    hist.SetBinContent(i,j,math.sqrt(hist.GetBinContent(i,j)))
		    #print i,j,hist.GetBinContent(i,j)
    ...
    def buildModel(self):
		...
		self.doHistFunc('xs_gg%sINT_%s' % (X, era), f.Get(hd['xs_gg%s'%X]), pars)
                self.doHistFunc('xs_qq%s_%s' % (X, era), f.Get(hd['xs_qq%s'%X]), pars)
                ...
                self.PROC_SETS.append(([ 'gg%s'%X, 'qq%s'%X], [ '%sWW'%X], [era])#(([ 'gg%s'%X, 'bb%s'%X ], [ '%stautau'%X, '%sbb'%X], [era])
