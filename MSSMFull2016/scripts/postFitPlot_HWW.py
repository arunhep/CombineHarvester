...
def fixBins(hist,dirname):
  if "top" in dirname or "dy" in dirname:
    pass
  elif "11" in dirname:
    hist.SetBins(hist.GetNbinsX(),array('d',[100,150,200,250,300,350,400,500,700,1000,2000]))
  else:
    hist.SetBins(hist.GetNbinsX(),array('d',[100,150,200,250,300,350,400,450,500,550,600,650,700,750,800,900,1000,2000]))
  return hist
...
#parser.add_argument('--r_qqH',default='0.1',help='Signal qqH XS*BR for model indep')
  ...
  for root,dirnames,filenames in os.walk(args.dir):
    ...
    for filename in fnmatch.filter(filenames, '*%(workspace)s.root'%vars()):
      ...
      if model_dep :
          ...
          shape_file=workspace_file.replace('.root','_shapes_mPhi%(mPhi)s_r_ggH%(r_ggH)s.root'%vars()) #_r_qqH%(r_qqH)s
          shape_file_name=filename.replace ('.root','_shapes_mPhi%(mPhi)s_r_ggH%(r_ggH)s.root'%vars()) #_r_qqH%(r_qqH)s
  
  if model_dep is True :
    print "using mA and tanb"
    freeze = 'mA='+mA+',tanb='+tb 
  else: 
    print "using MH="+mPhi+", r_ggH="+r_ggH#+" and r_qqH="+r_qqH
    freeze = 'MH='+mPhi+',r_ggH='+r_ggH#+',r_qqH='+r_qqH 
...
if not args.postfitshapes:
  if args.dir:
    for root,dirnames,filenames in os.walk(args.dir):
      ...
      else: filestring = '*_shapes_mPhi%(mPhi)s_r_ggH%(r_ggH)s.root'%vars()  #_r_qqH%(r_qqH)s
...
'em':[backgroundComp("tW and t#bar{t}",["top"],400),backgroundComp("DY", ["DY"], 418),backgroundComp("Fake",["Fake"],921),backgroundComp("WW",["WW","ggWW"],851),backgroundComp("VVV",["VVV"],857),backgroundComp("VZ",["VZ","WZ","ZZ"],617),backgroundComp("V#gamma^{(*)}",["Vg","Wg","VgS"],620),backgroundComp("Higgs 125 GeV",["H_htt","H_hww","ZH_hww","ggZH_hww","WH_hww","qqH_hww","ggH_hww"],632)],
'ee':[backgroundComp("tW and t#bar{t}",["top"],400),backgroundComp("DY", ["DY"], 418),backgroundComp("Fake",["Fake"],921),backgroundComp("WW",["WW","ggWW"],851),backgroundComp("VVV",["VVV"],857),backgroundComp("VZ",["VZ","WZ","ZZ"],617),backgroundComp("V#gamma^{(*)}",["Vg","Wg","VgS"],620),backgroundComp("Higgs 125 GeV",["H_htt","H_hww","ZH_hww","ggZH_hww","WH_hww","qqH_hww","ggH_hww"],632)],
'mm':[backgroundComp("tW and t#bar{t}",["top"],400),backgroundComp("DY", ["DY"], 418),backgroundComp("Fake",["Fake"],921),backgroundComp("WW",["WW","ggWW"],851),backgroundComp("VVV",["VVV"],857),backgroundComp("VZ",["VZ","WZ","ZZ"],617),backgroundComp("V#gamma^{(*)}",["Vg","Wg","VgS"],620),backgroundComp("Higgs 125 GeV",["H_htt","H_hww","ZH_hww","ggZH_hww","WH_hww","qqH_hww","ggH_hww"],632)]
...
if not model_dep: sighist_ggH = getHistogram(histo_file,'ggH_HWW',file_dir, mode, args.no_signal, log_x)[0]
#if not model_dep: sighist_qqH = getHistogram(histo_file,'qqH_HWW',file_dir, mode, args.no_signal, log_x)[0]
...
if auto_blind or auto_blind_check_only:
    ...
    #qqH_sigmas=[0.54,0.23,0.21,0.12,0.097,0.088,0.059,0.08,0.08,0.1,0.2,0.3]
    for root,dirnames,filenames in os.walk(args.dir):
      for filename in fnmatch.filter(filenames, '*.txt.cmb'):
        datacard_file = os.path.join(root,filename) 
      for filename in fnmatch.filter(filenames, '*%(workspace)s.root'%vars()):
        ...
        for i,p in enumerate(points) :
          ...
          freeze = 'MH='+str(p)+',r_ggH='+str(ggH_sigmas[i])#+',r_qqH='+str(qqH_sigmas[i])
          ...
          for j in range(1,bkghist.GetNbinsX()):
              ...
              if auto_blind_check_only: 
                  print "mPhi=",str(p)," r_ggPhi=",str(ggH_sigmas[i]), "BinLowEdge=", bkghist.GetBinLowEdge(j), "BinHighEdge=", bkghist.GetBinLowEdge(j+1), "ggH s/b:", soverb_ggH#, "qqH s/b:", soverb_qqH #," r_qqPhi=",str(qqH_sigmas[i])
              if soverb_ggH > 0.5:# or soverb_qqH > 0.5:
...
if not fractions:
  ...
  if not args.no_signal:
    if model_dep is True: 
      ...
      #sighist_qqH.SetLineColor(ROOT.kBlue + 3)
...
if not fractions:
  if not args.no_signal:
    if model_dep is True: 
        legend.AddEntry(sighist,"h,H,A#rightarrow WW"%vars(),"l")
    else: 
        legend.AddEntry(sighist_ggH,"gg#phi("+mPhi+")#rightarrow WW"%vars(),"l")
        #legend.AddEntry(sighist_qqH,"qq#phi("+mPhi+")#rightarrow#tau#tau"%vars(),"l")
  latex = ROOT.TLatex()
  latex.SetNDC()
  ...
  if not args.no_signal:
    if model_dep is True: 
        ...
        #latex.DrawLatex(0.65,0.52,"#sigma(qq#phi)=%(r_qqH)s pb"%vars())
...
if soverb_plot:
  ...
  if model_dep:
    ...
    #sighist_qqH_forratio.SetLineColor(ROOT.kBlue+3)
    #sighist_qqH_forratio.Draw("same")
